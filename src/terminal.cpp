...
void Terminal::connectSerialPort(Stream & serialPort, bool autoXONXOFF)
{
  ...
  if (m_keyboard->isKeyboardAvailable())
    xTaskCreatePinnedToCore(&keyboardReaderTask, "KRTSP1", FABGLIB_KEYBOARD_READER_TASK_STACK_SIZE, this, FABGLIB_KEYBOARD_READER_TASK_PRIORITY, &m_keyboardReaderTaskHandle, 1);
...
void Terminal::connectSerialPort(uint32_t baud, uint32_t config, int rxPin, int txPin, FlowControl flowControl, bool inverted)
{
  ...
  if (m_keyboard->isKeyboardAvailable())
    xTaskCreatePinnedToCore(&keyboardReaderTask, "KRTSP2", FABGLIB_KEYBOARD_READER_TASK_STACK_SIZE, this, FABGLIB_KEYBOARD_READER_TASK_PRIORITY, &m_keyboardReaderTaskHandle, 1);
}
